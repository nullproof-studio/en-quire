...
import { DocRenameSectionSchema, handleDocRenameSection } from './tools/write/doc-rename-section.js';
...
import { DocAssignIdsSchema, handleDocAssignIds } from './tools/write/doc-assign-ids.js';
...
export function registerEnQuireTools(registry: ToolRegistry): void {
  // Read tools
  registry.register({ name: 'doc_outline', description: 'Get the section structure of a document. Supports markdown (.md, .mdx — ATX headings), YAML (.yaml, .yml — keys at each level), and JSONL (.jsonl, .ndjson — one section per record, wrapped in a synthetic "__records" root for appending). Returns level, text, path, line numbers, and char count for each section. For markdown also returns word_count per section and total_word_count at root (fenced code blocks excluded), plus each section\'s stable "^id" anchor when it has one — use that id as a rename-proof address. Use this first to understand document structure before making changes. Returns an etag for use with write operations.', schema: DocOutlineSchema.shape, handler: handleDocOutline });
  registry.register({ name: 'doc_read_section', description: 'Read a specific section by heading text, path, or stable "^id" anchor (e.g. "^store-map"). Works for markdown headings, YAML keys (dot-path or "services > api > port"), and JSONL records (by index, e.g. "[5]", or by pattern like "*user*"). By default returns the section body AND all children. Set include_children=false to read only the body text (before the first child heading). Use doc_outline first to find the correct section address. Returns an etag for use with write operations.', schema: DocReadSectionSchema.shape, handler: handleDocReadSection });
  ...
  // Write tools
  registry.register({ name: 'doc_replace_section', description: 'Replace a section\'s content. The heading is preserved automatically — do NOT include it in content. IMPORTANT: if content contains subsection headings (e.g. ### child), the body AND all existing children are replaced with the new content. If content is plain text (no subsection headings), only the body is replaced and children are preserved. WARNING: replacing a top-level (h1) section with subsection headings replaces the entire document body beneath that heading — use doc_outline to check structure first. For correcting specific text within a section, prefer doc_find_replace — it is more precise and avoids unintended formatting changes. To REMOVE a section entirely (heading + body + children), use doc_delete_section — replacing with empty content only clears the body and leaves an orphan heading. To rename a heading and nothing else, use doc_rename_section — it needs no body and cannot clobber one. When replace_heading is true, content must include the full heading line (e.g. "## New Title\\nBody") AND the entire body, because the section is replaced from heading start to body end; anything you omit is lost. Pass if_match (from a prior read) to prevent stale writes.', schema: DocReplaceSectionSchema.shape, handler: handleDocReplaceSection });
  ...
  registry.register({ name: 'doc_delete_section', description: 'Delete a section entirely — its heading, body, AND all children — in one call. This is the right tool for removing a section (do NOT replace with empty content, which leaves an orphan heading). WARNING: deleting a top-level (h1) section removes the entire document body beneath that heading. Use doc_outline to check what will be affected. To remove several sections, call repeatedly: the response returns a fresh etag you can pass as the next call\'s if_match to chain deletes without re-reading. Pass if_match (from a prior read) to prevent stale writes.', schema: DocDeleteSectionSchema.shape, handler: handleDocDeleteSection });
  registry.register({ name: 'doc_rename_section', description: 'Rename a section\'s heading, leaving its body and children completely untouched. This is the tool for changing a heading — it takes no content parameter, so a rename can never overwrite the body. One call, no prior read of the body needed, and the heading level plus the stable "^id" anchor are preserved (so existing addresses and inbound [[doc#^id]] links stay valid). Use doc_replace_section only when you are deliberately rewriting the body at the same time. Markdown only. Fails if a sibling already has the new heading. Pass if_match (from a prior read) to prevent stale writes.', schema: DocRenameSectionSchema.shape, handler: handleDocRenameSection });
  ...
  registry.register({ name: 'doc_create', description: 'Create a new document. Fails if the file already exists — existing files are always edited in place, never recreated: doc_find_replace or doc_replace_section for content, doc_replace_section with replace_heading for heading renames, doc_move_section to reorder or re-nest sections, doc_insert_section / doc_delete_section to add or remove them, and doc_rename to move the file to a different path. Restructuring an existing document never requires archiving it and creating a replacement. Content is validated for structural issues before writing. Returns an etag for the new document.', schema: DocCreateSchema.shape, handler: handleDocCreate });
  ...
  registry.register({ name: 'doc_rename', description: 'Rename a document within the same root. Source must exist and destination must not. Renames the file only — it does not rename a heading (for that, use doc_replace_section with replace_heading). Cross-root rename is not supported: you can copy content across with doc_read + doc_create, but en-quire has no whole-file delete, so the original must be removed outside en-quire. Pass if_match (from a prior read) to prevent stale writes.', schema: DocRenameSchema.shape, handler: handleDocRename });
  ...
  registry.register({ name: 'doc_assign_ids', description: 'Backfill stable "^id" anchors onto every markdown heading that lacks one (deriving each id from the heading text). Existing anchors are preserved. Use this to migrate older documents, or any time you want durable, rename-proof section addresses: once a heading has an anchor, address it as "^id" and link to it as [[doc#^id]] — the reference survives renames, renumbering, and moves. Returns the list of headings that were assigned. Markdown only. Pass if_match (from a prior read) to prevent stale writes.', schema: DocAssignIdsSchema.shape, handler: handleDocAssignIds });
