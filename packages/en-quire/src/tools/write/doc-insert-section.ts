...
export const DocInsertSectionSchema = z.object({
  ...
  position: enumWithHint(
    ...
    "This tool only inserts new sections. To replace an existing section's content, use doc_replace_section instead.",
  ).describe('"before"/"after" insert as a sibling of the anchor. "child_start"/"child_end" insert as a child of the anchor. Fails if a sibling with the same heading already exists — use doc_replace_section to update existing sections.'),
  heading: z.string().describe('Plain text heading without # markers (e.g. "My Section", not "## My Section"). The heading level is set automatically from position context or the level parameter. Keep headings short and stable — this text becomes the section\'s address for every later read and edit. Put status, qualifiers, or parentheticals (e.g. "(pending OQ-004)") in the body, not the heading: such suffixes are content, and baking them into the heading makes the section harder to address and the address drift when they change.'),
