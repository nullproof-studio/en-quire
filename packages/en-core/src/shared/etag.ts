...
// BIP-0039 english wordlist (2048 words, MIT licensed).
// Source: https://github.com/bitcoin/bips/blob/master/bip-0039/english.txt
// See THIRD-PARTY-LICENSES for attribution.
// prettier-ignore
const WORDS = [
 ...
 * Returns two hyphen-joined words from the BIP-0039 wordlist (e.g. "orbit-clerk")
 * derived from SHA-256 hash bytes. Agents reproduce whole words reliably, unlike
 * character-level encodings (hex/base64) where single characters get dropped or
 * swapped during tool-call round-trips.
 *
 * Two words from 2048 = 22 bits of collision space (4.2M combinations), sufficient
 * for stale-read detection.
 */
export function computeEtag(content: string): string {
  const hash = createHash('sha256').update(content).digest();
  // Each word index is 11 bits; pack two from the first 3 bytes.
  const bits = (hash[0]! << 16) | (hash[1]! << 8) | hash[2]!;
  const w1 = (bits >> 11) & 0x7ff;
  const w2 = bits & 0x7ff;
  return `${WORDS[w1]}-${WORDS[w2]}`;
}
