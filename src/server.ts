...
export function createServer(deps: ServerDependencies): McpServer {
  ...
  server.tool('doc_move_section', 'Move a section (heading + body + children) to a new position within the same document. Heading levels are adjusted automatically — e.g. moving an h2 to become a child of another h2 makes it h3, and its children shift accordingly. This is atomic: the section is extracted and re-inserted in one operation, avoiding data loss. Fails if the destination already has a sibling with the same heading, or if you try to move a section into its own subtree. IMPORTANT: do not number headings (e.g. avoid "2. Background") — the system does not auto-renumber when sections are reordered.', DocMoveSectionSchema.shape, wrapHandler(handleDocMoveSection));
