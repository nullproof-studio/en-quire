...
export function createServer(deps: ServerDependencies): McpServer {
  ...
  server.tool('doc_insert_text', 'Insert a new paragraph at a specific position within a section body, anchored to existing text. Use when you want to add prose between two existing paragraphs or at the start/end of a paragraph — not when adding a new subsection (use doc_insert_section) or appending to the end of a section body (use doc_append_section). The anchor must be a distinctive literal string that appears exactly once in the document; short anchors at paragraph boundaries (first/last few words of a paragraph) work best. If the anchor is ambiguous, the error returns each candidate match with its section path and line number so you can pick a longer/more distinctive anchor. If you want to replace existing text rather than insert new text, use doc_find_replace.', DocInsertTextSchema.shape, wrapHandler('doc_insert_text', handleDocInsertText));
