...
export function createServer(deps: ServerDependencies): McpServer {
  ...
  server.tool('doc_insert_section', 'Insert a new section relative to an anchor section. Heading must be plain text without # markers (e.g. "My Section", not "## My Section") — level is set automatically or via the level parameter. IMPORTANT: do not number headings (e.g. avoid "2. Background" or "Section 3: Design") — the system does not auto-renumber when sections are inserted or reordered, which leads to incorrect numbering. Use descriptive names instead (e.g. "Background", "Design"). Fails if a sibling with the same heading already exists; use doc_replace_section to update existing sections. Not supported for YAML files.', DocInsertSectionSchema.shape, wrapHandler(handleDocInsertSection));
  ...
  server.tool('doc_move_section', 'Atomically delete a section from its current location and insert it at a new position — equivalent to cut-and-paste. Heading levels adjust automatically including all children (e.g. moving an h2 to become a child of another h2 makes it h3). Use this instead of separate delete + insert calls, which risk data loss when section content exceeds context window. Fails if the destination already has a sibling with the same heading, or if you try to move a section into its own subtree. IMPORTANT: do not number headings (e.g. avoid "2. Background") — the system does not auto-renumber when sections are reordered.', DocMoveSectionSchema.shape, wrapHandler(handleDocMoveSection));
